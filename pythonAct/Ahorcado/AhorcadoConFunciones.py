...
def guardo_posiciones(pal_separada, pal, cantidades, letra):
...
def armo_palabra(pal_separada):
    pal_imprime = ''
    for y in pal_separada:
        pal_imprime = pal_imprime + y[0]
    print(pal_imprime)

def si_gane(letras_a, pal):
    if cantidades['letras adivinadas']== len(pal):
        print ('Ganaste')
        return False
    else:
        return True

def letra_inc(cantidades):
    cantidades['partes cuerpo']=cantidades['partes cuerpo'] + 1
    for x in range(cantidades['partes cuerpo']):
        print (ahorcado[x])
    if cantidades['partes cuerpo']== 3:
...
def un_intento(pal, pal_separada, ahorcado, cantidades):
    ...
    if letra in pal:
        pal_separada=guardo_posiciones(pal_separada, pal, cantidades, letra)
        armo_palabra(pal_separada)
        return si_gane(cantidades['letras adivinadas'], pal)
    else:
        return letra_inc(cantidades)
